CREATE TABLE IF NOT EXISTS blade_dds.countries (
...
select 		ods.country_id 
			...
			, 'I' as md_dwh_status
from 		blade_ods.countries ods
full join	blade_dds.countries dds
on			ods.country_id = dds.country_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.countries
...
and 		ods.country_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.country_id::text, '') ||
			coalesce(ods.country_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.country_id::text, '') ||
					coalesce(dds.country_name::text, '')
					)
) as sbq
WHERE blade_dds.countries.dwh_id=sbq.dwh_id;
