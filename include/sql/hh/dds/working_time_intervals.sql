CREATE TABLE IF NOT EXISTS blade_dds.working_time_intervals (
...
-- (1) new rows
insert into blade_dds.working_time_intervals (
working_time_intervals_id, working_time_intervals_name, 
md_ins_date, md_upd_date, md_is_activ, md_is_delete, 
md_dwh_is_activ, md_dwh_date_from, md_dwh_date_to, md_dwh_status
)
select 		ods.working_time_intervals_id 
			, ods.working_time_intervals_name 
			, ods.md_ins_date 
			, ods.md_upd_date 
			, ods.md_is_activ 
			, ods.md_is_delete 
			...
			, 'I' as md_dwh_status
from 		blade10.blade_ods.working_time_intervals ods
full join	blade10.blade_dds.working_time_intervals dds
on			ods.working_time_intervals_id = dds.working_time_intervals_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.working_time_intervals
...
from 		blade_ods.working_time_intervals ods
full join	blade_dds.working_time_intervals dds
on			ods.working_time_intervals_id = dds.working_time_intervals_id
where 		1=1
and 		dds.md_dwh_is_activ = 1
and			dds.md_dwh_status != 'D' 	
and 		ods.working_time_intervals_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.working_time_intervals_id::text, '') ||
			coalesce(ods.working_time_intervals_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.working_time_intervals_id::text, '') ||
					coalesce(dds.working_time_intervals_name::text, '')
					)
) as sbq
WHERE blade_dds.working_time_intervals.dwh_id=sbq.dwh_id;
