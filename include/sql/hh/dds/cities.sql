CREATE TABLE IF NOT EXISTS blade_dds.cities (
...
and  		md5(
			...
			coalesce(ods.city_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.city_id::text, '') ||
					coalesce(dds.city_name::text, '')
					)
) as sbq
WHERE blade_dds.cities.dwh_id=sbq.dwh_id;
