CREATE TABLE IF NOT EXISTS blade_dds.schedule (
...
select 		ods.schedule_id 
			...
			, 'I' as md_dwh_status
from 		blade10.blade_ods.schedule ods
full join	blade10.blade_dds.schedule dds
on			ods.schedule_id = dds.schedule_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.schedule
...
from 		blade_ods.schedule ods
full join	blade_dds.schedule dds
on			ods.schedule_id = dds.schedule_id
where 		1=1
and 		dds.md_dwh_is_activ = 1
and			dds.md_dwh_status != 'D' 	
and 		ods.schedule_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.schedule_id::text, '') ||
			coalesce(ods.schedule_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.schedule_id::text, '') ||
					coalesce(dds.schedule_name::text, '')
					)
) as sbq
WHERE blade_dds.schedule.dwh_id=sbq.dwh_id;
