CREATE TABLE IF NOT EXISTS blade_dds.education_level (
...
-- (1) new rows
insert into blade_dds.education_level (
...
select 		ods.education_level_id 
			, ods.education_level_name 
			, ods.md_ins_date 
			, ods.md_upd_date 
			, ods.md_is_activ 
			, ods.md_is_delete 
			...
			, 'I' as md_dwh_status
from 		blade10.blade_ods.education_level ods
full join	blade10.blade_dds.education_level dds
on			ods.education_level_id = dds.education_level_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.education_level
...
from 		blade_ods.education_level ods
full join	blade_dds.education_level dds
on			ods.education_level_id = dds.education_level_id
where 		1=1
and 		dds.md_dwh_is_activ = 1
and			dds.md_dwh_status != 'D' 	
and 		ods.education_level_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.education_level_id::text, '') ||
			coalesce(ods.education_level_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.education_level_id::text, '') ||
					coalesce(dds.education_level_name::text, '')
					)
) as sbq
WHERE blade_dds.education_level.dwh_id=sbq.dwh_id;
