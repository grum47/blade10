CREATE TABLE IF NOT EXISTS blade_dds.employer_type (
    dwh_id serial PRIMARY KEY, 
    employer_type_id text, 
    employer_type_name text, 
    md_ins_date date, 
    md_upd_date date, 
    md_is_activ integer, 
...
-- (1) new rows
insert into blade_dds.employer_type (
employer_type_id, employer_type_name, 
md_ins_date, md_upd_date, md_is_activ, md_is_delete, 
md_dwh_is_activ, md_dwh_date_from, md_dwh_date_to, md_dwh_status
)
select 		ods.employer_type_id 
			, ods.employer_type_name 
			, ods.md_ins_date 
			, ods.md_upd_date 
			, ods.md_is_activ 
			, ods.md_is_delete 
			...
			, 'I' as md_dwh_status
from 		blade10.blade_ods.employer_type ods
full join	blade10.blade_dds.employer_type dds
on			ods.employer_type_id = dds.employer_type_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.employer_type
...
from 		blade_ods.employer_type ods
full join	blade_dds.employer_type dds
on			ods.employer_type_id = dds.employer_type_id
where 		1=1
and 		dds.md_dwh_is_activ = 1
and			dds.md_dwh_status != 'D' 	
and 		ods.employer_type_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.employer_type_id::text, '') ||
			coalesce(ods.employer_type_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.employer_type_id::text, '') ||
					coalesce(dds.employer_type_name::text, '')
					)
) as sbq
WHERE blade_dds.employer_type.dwh_id=sbq.dwh_id;
