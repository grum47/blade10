CREATE TABLE IF NOT EXISTS blade_dds.working_days (
...
-- (1) new rows
insert into blade_dds.working_days (
...
select 		ods.working_days_id 
			, ods.working_days_name 
			, ods.md_ins_date 
			, ods.md_upd_date 
			, ods.md_is_activ 
			, ods.md_is_delete 
			...
			, 'I' as md_dwh_status
from 		blade10.blade_ods.working_days ods
full join	blade10.blade_dds.working_days dds
on			ods.working_days_id = dds.working_days_id
where 		1=1 
and 		dds.dwh_id is null;

-- (2) update old rows
update 	blade_dds.working_days
...
from 		blade_ods.working_days ods
full join	blade_dds.working_days dds
on			ods.working_days_id = dds.working_days_id
where 		1=1
and 		dds.md_dwh_is_activ = 1
and			dds.md_dwh_status != 'D' 	
and 		ods.working_days_id is not null
and 		dds.dwh_id is not null
and  		md5(
			'' ||
			coalesce(ods.working_days_id::text, '') ||
			coalesce(ods.working_days_name::text, '')
			) != md5(
					'' ||
					coalesce(dds.working_days_id::text, '') ||
					coalesce(dds.working_days_name::text, '')
					)
) as sbq
WHERE blade_dds.working_days.dwh_id=sbq.dwh_id;
